"""Modificado por OagronomIA (2026-09-23) a partir de nkarasiak/qgis-mcp v0.15.0 — GPLv2+: o script
exportado chama o executor da ClaudIA QGIS (``plugins["claudia_qgis"].executor``) em vez do servidor de socket.
...
_SCRIPT_HEADER = '''\
"""Replay of a ClaudIA QGIS session, exported {created}.

Run it from the QGIS Python console (Plugins > Python Console > Show Editor,
open, Run) with the ClaudIA QGIS plugin loaded and connected. It stops at
...
_server = plugins["claudia_qgis"].executor
if _server is None:
    raise RuntimeError("Conecte o plugin ClaudIA QGIS à comunidade primeiro (Complementos > ClaudIA QGIS)")
...
class SessionHandlers:
    ...
    def restore_checkpoint(self, checkpoint_id, **kwargs):
        ...
        if getattr(self, "iface", None):  # ClaudIA QGIS: sem iface no smoke headless
