...
class SystemHandlers:
    ...
    _CODE_TIMEOUT = 55
    _CODE_FILENAME = "<claudia_qgis execute_code>"
    ...
    def diagnose(self, **kwargs):
        ...
        # 3. Conexão com a comunidade agrônomo10X (ClaudIA QGIS — preenchido pelo plugin)
        comunidade = dict(getattr(self, "comunidade", {}) or {})
        checks.append(
            {
                "name": "comunidade",
                "status": "ok" if comunidade.get("conectado") else "degraded",
                "detail": comunidade,
            }
        )
